	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for idx := range PackConf.File.Deb {
			...
			if PackConf.File.Deb[idx].Ref == "" {
				if ret, _, err := comm.ExecAndWait(10, "apt", "download", PackConf.File.Deb[idx].Id, "-y", "--print-uris"); err == nil {
					url := strings.Split(ret, " ")[0]
					PackConf.File.Deb[idx].Ref = strings.Replace(url, "'", "", 2)
				}
			}
			...
			if len(PackConf.File.Deb[idx].Ref) > 0 {
				...
				builder := linglong.LinglongBuder{
					...
					Kind:        PackConf.File.Deb[idx].Type,
					Hash:        PackConf.File.Deb[idx].Hash,
					Url:         PackConf.File.Deb[idx].Ref,
